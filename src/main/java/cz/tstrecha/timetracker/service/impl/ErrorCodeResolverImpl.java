...
public class ErrorCodeResolverImpl implements ErrorCodeResolver {
    ...
    public String resolveException(ErrorTypeCode errorTypeCode){
        ...
        if (ClassLoader.getSystemResource(STR."error_codes_\{locale.getLanguage()}.properties") == null){
    ...
    public ResponseEntity<ErrorDTO> resolveException(LocalizedException exception) {
        var errorDetail = new ErrorDTO();

        errorDetail.setCode(exception.getErrorTypeCode());
